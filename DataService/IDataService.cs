...
namespace rawdata_portfolioproject_2
{
    public interface IDataService
    {
        ...
        IList<weighted_inverted_index> RankedWeightSearch(PagingAttributes pagingattributes, params string[] keywords);
        Profile CreateProfile(string email, string password);
        Profile GetProfile(int profileId);
        Profile GetProfile(string email);
        bool UpdateProfileEmail(int profileId, string email);
        bool UpdateProfilePassword (int profileId, string password);
